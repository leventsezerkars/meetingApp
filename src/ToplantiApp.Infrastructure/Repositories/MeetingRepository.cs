...
public class MeetingRepository : GenericRepository<Meeting>, IMeetingRepository
{
    ...
    public async Task<IReadOnlyList<Meeting>> GetByUserIdAsync(int userId)
        => await _dbSet
            .Include(m => m.CreatedBy)
            .Include(m => m.Participants)
    ...
    public async Task<IReadOnlyList<Meeting>> GetCancelledMeetingsOlderThanAsync(DateTime date)
        => await _dbSet
